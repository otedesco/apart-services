...
async function findOne(argsObject: Partial<Account>) {
  return Cache.cacheSimpleResponse(
    idColumns, Accounts.tableName, SESSION_EXPIRE, ENABLED_CACHE, AccountRepository.findOne, argsObject,
  );
}

async function create(argsObject: Partial<Account>, tx?:Transaction) {
  return Cache.cacheSimpleResponse(
    idColumns, Accounts.tableName, SESSION_EXPIRE, false, 
    args => AccountRepository.create(args, tx), argsObject,
  );
}
