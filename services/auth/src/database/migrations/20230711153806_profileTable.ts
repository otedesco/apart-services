...
import { ACCOUNT_TABLE, PROFILE_TABLE, PROFILE_TYPE_TABLE, ROLE_TYPE_TABLE } from '../../configs/DBConfig';
...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable(PROFILE_TABLE, table => {
    ...
    table.string('role').notNullable().defaultTo(RoleTypeEnum.OWNER).references('role').inTable(ROLE_TYPE_TABLE);
