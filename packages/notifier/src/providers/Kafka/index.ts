...
const producerKey = (topic: string) => `${process.env.PRODUCER_TOPIC_PREFIX}_${topic}`;

const getProducer = (topic: string) => (process.env.PRODUCE_EVENTS ? _.get(producersPerTopic, topic, null) : null);

const getConfig = () => ({ host: process.env.KAFKA_HOST_MICROSERVICES || 'localhost:9092' });
const startProducer = (topic: string, config = getConfig()) => {
  const producerInstance = new ClientProducer({ topic, ...config });
...
const sendMessage = async <T>(topic: string, message: T) => {
  ...
  const producerInstance = await getProducer(prefixedTopic) || await startProducer(prefixedTopic);
  producerInstance?.producer.logger().info(
    `New outgoing message for topic: ${prefixedTopic}. Message: ${JSON.stringify(message)}`,
  );
